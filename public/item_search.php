...
<body>
<?php
$stmt = "SELECT * FROM `items` as `i` LEFT JOIN (SELECT `parent_item_id`,COUNT(1) as `reagent_count` FROM `craft_links` GROUP BY `parent_item_id`) `t` ON `t`.`parent_item_id`=`i`.`id` WHERE `name` LIKE '%" . $_GET['query'] . "%'";
...
function ingredientsTable($id, $multiplier = 1, $nesting = 0)
{
    ...
    $stmt = "SELECT `items`.`name`,`craft_links`.`amount`,`parent_name`,`items`.`id` FROM (SELECT `name` as `parent_name` FROM `items` WHERE `id`='" . $id . "') k, `craft_links` LEFT JOIN `items` ON `items`.`id`=`craft_links`.`child_item_id` WHERE `parent_item_id`='" . $id . "'";
...
{$nesting_str}					<img class="item_image" alt="{$ingredient['name']}" />
{$nesting_str}				</div>
